...
class Gdpr_translate
{
    ...
    public function __construct()
    {
        ...
        $this->module = Module::getInstanceByName('ets_gdpr');
    }

    public function l($string, $source = null)
    {
        return Translate::getModuleTranslation('ets_gdpr', $string, $source !== null ? $source : pathinfo(__FILE__, PATHINFO_FILENAME));
    }
    ...
    public static function trans($text, $iso_code, $specific = null)
    {
        ...
        $files_by_priority = _PS_MODULE_DIR_ . 'ets_gdpr/translations/' . $iso_code . '.php';
        if (!@file_exists($files_by_priority)) {
            return $text;
        }
        $string = preg_replace("/\\\*'/", "\'", $text);
        $key = md5($string);
        $default_key = Tools::strtolower('<{ets_gdpr}prestashop>' . ($specific ? Tools::strtolower($specific) : 'ets_gdpr')) . '_' . $key;
