...
<div class="panel tab-pane fade {if empty($section ) || $section=='settings'}active in{/if}" id="settings" role="tabpanel" aria-labelledby="settings-tab">
    <div>
        <div class="form-wrapper">
            ...
            <div class="form-group">
                <label class="control-label col-lg-3">{l s='Enable Cashback Reward Banner?' mod='revolutpayment'}</label>
                <div class="col-lg-9">
					<span class="switch prestashop-switch fixed-width-lg">
						<input type="radio" name="REVOLUT_PAY_BANNER_ENABLE" id="REVOLUT_PAY_BANNER_ENABLE_on" value="1"
                               {if $REVOLUT_PAY_BANNER_ENABLE == 1}checked="checked"{/if} />
						<label for="REVOLUT_PAY_BANNER_ENABLE_on">{l s='Yes' mod='revolutpayment'}</label>
						<input type="radio" name="REVOLUT_PAY_BANNER_ENABLE" id="REVOLUT_PAY_BANNER_ENABLE_off"
                               value="0" {if $REVOLUT_PAY_BANNER_ENABLE == 0}checked="checked"{/if} />
						<label for="REVOLUT_PAY_BANNER_ENABLE_off">{l s='No' mod='revolutpayment'}</label>
						<a class="slide-button btn"></a>
					</span>
                    <p class="help-block">{l s='Customers can get instructions to signup to Revolut and get rewarded.' mod='revolutpayment'}</p>
                </div>
            </div>
