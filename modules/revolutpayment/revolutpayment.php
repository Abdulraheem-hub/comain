...
class RevolutPayment extends PaymentModule
{
    ...
    public function install()
    {
        ...
        $default_revolut_reward_banner = 1;
        ...
        if (Configuration::get('REVOLUT_PAY_BANNER_ENABLE') == null) {
            Configuration::updateValue('REVOLUT_PAY_BANNER_ENABLE', $default_revolut_reward_banner);
        }
    ...
    public function postProcess()
    {
        ...
        $revolutBannerEnabled = (bool) Tools::getValue('REVOLUT_PAY_BANNER_ENABLE');
        ...
        Configuration::updateValue('REVOLUT_PAY_BANNER_ENABLE', $revolutBannerEnabled);
    ...
    public function displayRevolutSettingsPage()
    {
        ...
        $this->context->smarty->assign(
            [
                ...
                'REVOLUT_PAY_BANNER_ENABLE' => Configuration::get('REVOLUT_PAY_BANNER_ENABLE'),
    ...
    public function hookHeaderV17()
    {
        ...
        if ($this->context->controller->php_self == 'order' || Tools::getValue('module') == 'revolutpayment') {
            ...
            $revolut_order_public_id = $this->getRevolutOrderByIdCart($id_cart);
            $amex_availability = false;
            if (isset($revolut_order_public_id['public_id'])) {
    ...
    public function hookPayment($params)
    {
        ...
        $this->cardWidgetIsEnabled = in_array('card', $payment_option_availability);
        $this->payWidgetIsEnabled = in_array('pay_with_revolut', $payment_option_availability);
        $payment_options_not_available = (!$this->cardWidgetIsEnabled && !$this->payWidgetIsEnabled);

        if ($payment_options_not_available) {
            return '';
        }

        $this->context->smarty->assign(
            [
                'is_banner_enabled' => Configuration::get('REVOLUT_PAY_BANNER_ENABLE'),
    ...
    public function hookDisplayOrderConfirmation($params)
    {
        ...
        $isBannerEnabled = Configuration::get('REVOLUT_PAY_BANNER_ENABLE');

        if (!$order || !$isBannerEnabled) {
            return;
        }
        ...
        return $this->display(__FILE__, '/views/templates/hook/upsell_banner.tpl');
    }
