...
trait RevolutModuleHelper
{
    ...
    public function collectLineItems($ps_order_id)
    {
        ...
        foreach ($products as $product) {
            ...
            $image_urls = !empty($this->getProductImages($product_obj)) ? $this->getProductImages($product_obj) : null;
            ...
            if (!empty($tax_amount)) {
                $taxes = [
                    'name' => $tax_name,
                    'amount' => $this->createRevolutAmount($tax_amount, $currency),
                ];
            }
    ...
    public function getProductImages($product)
    {
        try {
            $id_lang = $this->context->language->id;
            $image_urls = array_map(function ($image_ids) use ($product, $id_lang) {
                return $this->context->link->getImageLink(
                    isset($product->link_rewrite[$id_lang]) ? $product->link_rewrite[$id_lang] : $product->name[$id_lang],
                    $image_ids['id_image'],
                    $this->isPs8 ? ImageType::getFormattedName('large') : ImageType::getFormatedName('large')
                );
            }, $product->getImages($id_lang));

            return $image_urls;
        } catch (Exception $e) {
            PrestaShopLogger::addLog('Unable to generate product image urls' . $e->getMessage(), 3);
